...
axios
  ...
  .then((res) => {
    ...
    // 获取内容 渲染页面
    let infoName = res.data.data
    // 欢迎 优先显示nickname 否则显示username
    let name = infoName.nickname || infoName.username
    $('#welcome').text('欢迎 ' + name)
    // 显示头像 优先显示头像  否则显示文字头像
    if (infoName.user_pic) {
      $('.layui-nav-img avatar').attr('src', infoName.user_pic).show()
      $('.text-avatar').hide()
    } else {
      $('.text-avatar-box').show().children().text(name[0].toUpprerCase)
    }
  })
